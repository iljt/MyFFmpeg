...
Java_com_example_player_MainActivity_getFFmpegVersion(
        JNIEnv *env,
        jobject /* this */) {
...
Java_com_example_player_MyFFmpegPlayer_prepareNative(JNIEnv *env, jobject job, jstring data_source) {
    const char * data_source_ = env->GetStringUTFChars(data_source, 0);
    auto *helper = new JNICallbakcHelper(vm, env, job); // C++子线程回调 ， C++主线程回调
    player = new MyFFmpegPlayer(data_source_, helper);
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_player_MyFFmpegPlayer_startNative(JNIEnv *env, jobject thiz) {
    if (player) {
        player->start();
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_player_MyFFmpegPlayer_stopNative(JNIEnv *env, jobject thiz) {
    if (player) {
        player->stop();
    }
}
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_player_MyFFmpegPlayer_releaseNative(JNIEnv *env, jobject thiz) {
    pthread_mutex_lock(&mutex);
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_player_MyFFmpegPlayer_setSurfaceNative(JNIEnv *env, jobject thiz, jobject surface) {
    pthread_mutex_lock(&mutex);
...
Java_com_example_player_MyFFmpegPlayer_getDurationNative(JNIEnv *env, jobject thiz) {
    if (player) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_player_MyFFmpegPlayer_seekNative(JNIEnv *env, jobject thiz, jint play_value) {
    if (player) {
