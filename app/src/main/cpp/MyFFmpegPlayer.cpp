...
MyFFmpegPlayer::MyFFmpegPlayer(const char *data_source, JNICallbakcHelper *helper) {
    ...
    pthread_mutex_init(&seek_mutex, nullptr); // 第七次增加 3.1
}
...
MyFFmpegPlayer::~MyFFmpegPlayer() {
    ...
    pthread_mutex_destroy(&seek_mutex); // 第七次增加 3.1
}

// TODO >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  下面全部都是 prepare

void *task_prepare(void *args) { // 此函数和MyFFmpegPlayer这个对象没有关系，你没法拿MyFFmpegPlayer的私有成员

    // avformat_open_input(0, this->data_source)

    auto *player = static_cast<MyFFmpegPlayer *>(args);
    player->prepare_();
    return nullptr; // 必须返回，坑，错误很难找
}

void MyFFmpegPlayer::prepare_() { // 此函数 是 子线程
...
// TODO 第五次 内存泄漏关键点（控制packet队列大小，等待队列中的数据被消费） 1
// 把 视频 音频 的压缩包(AVPacket *) 循环获取出来 加入到队列里面去
void MyFFmpegPlayer::start_() { // 子线程
    while (isPlaying) {
        ...
        if (!ret) { // ret == 0
            ...
            if (video_channel && video_channel->stream_index == packet->stream_index) {
                ...
                // 代表是音频
                audio_channel->packets.insertToQueue(packet); // TODO 第四次增加的
            }
...
void MyFFmpegPlayer::setRenderCallback(RenderCallback renderCallback) {
    this->renderCallback = renderCallback;
}

// TODO 第七次增加 获取总时长
int MyFFmpegPlayer::getDuration() {
    return duration; // 在调用此函数之前，必须给此duration变量赋值
}

void MyFFmpegPlayer::seek(int progress) {
     ...
     */
     ...
     /**
      *   av_seek_frame为 libavformat.a里面的一个方法
      *    formatContext媒体上下文 封装格式
      *    AV_TIME_BASE 时间基
      *    progress 拖动条progress 从java传过来的值为 progress * 100 / duration 以秒计算seekBar相对总时长的百分比
      *
      *    AVSEEK_FLAG_ANY直接精准到 拖动的位置，问题：如果不是关键帧，可能会造成 花屏情况
      *    AVSEEK_FLAG_BACKWARD（找附件的关键帧 ，如果找不到他也会花屏）
...
void MyFFmpegPlayer::stop_(MyFFmpegPlayer * myFFmpegPlayer) {
    isPlaying = false;
    ...
    DELETE(myFFmpegPlayer);
}

void MyFFmpegPlayer::stop() {
    ...
    // 我们要等这两个线程 稳稳的停下来后，我再释放MyFFmpegPlayer的所以工作
    // 由于我们要等 所以会ANR异常
